...
- **ai_player.py (`AIPlayer`)** — The AI opponent on the right side of the map. Has its own `ResourceManager`, buildings, units, and mineral nodes. Draws with orange tint. Uses the **command-based architecture**: `think()` generates command dicts (same format as multiplayer), `drain_commands()` returns them, and `game.py` executes them via `execute_command()`. `update_simulation()` handles reactive logic (auto-targeting, production ticks, cleanup). Configurable via AI profiles in `ai_profiles/`.
- **player_ai.py (`PlayerAI`)** — Only active in `--playforme` spectator mode. Controls the *player's* entities via commands (same pattern as AIPlayer: `think()` → `drain_commands()` → `execute_command()`).
...
- **commands.py** — Serializable command definitions (move, queue_waypoint, mine, place_building, train_unit, train_scout, attack, repair) and `execute_command()` engine that applies commands to either team's entities. Used by both multiplayer network commands and single-player AI commands.
...
7. `WaveManager.update()` and `AIPlayer.update_simulation()` (or `RemotePlayer.update_simulation()` in multiplayer). AI strategic decisions happen earlier via `think()` + `drain_commands()` in `game.py`.
8. Dead unit/building removal, win/lose checks.

### Entity model

- **Units** (`units.py`): `Unit` base → `Soldier`, `Scout`, `Tank`, `Worker`, `Yanuses`. All have waypoint movement, HP, selection, combat attributes, `net_id`. `Scout` is a fast, cheap recon unit trained from the Barracks. `Worker` has states: `idle`, `moving_to_mine`, `waiting`, `mining`, `returning`, `deploying`, `repairing`. Workers can repair damaged entities at 5 HP/sec. Only one worker can mine a node at a time. Units have vision range (1.2x attack range) and hunt visible enemies.
- **Buildings** (`buildings.py`): `Building` base → `TownCenter`, `Barracks`, `Factory`, `DefenseTower`, `Watchguard`, `Radar`. Production buildings have queues with timers. `DefenseTower` has `combat_update()` for auto-targeting. `Watchguard` expands placement zone by 500px and consumes its builder worker. `Radar` provides extended vision range (5000px) for fog of war. All buildings have `net_id` and `build_time` (construction takes time).
...
| Input | Action |
|-------|--------|
| T / B / F / D / G / R | Place Town Center / Barracks / Factory / Defense Tower / Watchguard / Radar |
...
| S | Toggle background music on/off |
